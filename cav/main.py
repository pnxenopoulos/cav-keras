...
class TCAV():
    ...
    def split_model(self, bottleneck, conv_layer = True):
        ...
        if conv_layer:
            ...
            self.model_h.add(InputLayer(input_shape = self.model_f.layers[bottleneck + 1].output_shape[1:]))
            ...
            self.model_h.add(InputLayer(input_shape = self.model.layers[bottleneck + 1].input_shape[1:]))
            for current_layer in range(bottleneck + 1, len(self.model.layers)):
                self.model_h.add(self.model.layers[current_layer])
    def _create_counterexamples(self, x_concept):
        ...
        counterexamples = []
        for i in range(0, n):
            counterexamples.append(np.rint(np.random.rand(height, width, channels) * 255))
